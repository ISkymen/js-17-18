...
var data = function () {
    var products = getData('http://10.skm.pp.ua/js-11-12/products.html', 'text');
    var prodArray = [];
    var prodBarcode = [];
    var prodName = [];
    var prodImage = [];
    for (var i = 0; i < products.length; i++) {
        prodArray.push(getData('http://dobavkam.net/barcode/' + products[i], 'json'));
        prodBarcode.push(products[i]);
        prodName.push(prodArray[i][0].product.name);
        prodImage.push(prodArray[i][0].product.image.src);
    }
